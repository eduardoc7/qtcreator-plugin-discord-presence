...
void QDiscordRichPresence::UpdateRichPresence() const {
  ...
  presence.largeImageKey =
      large_image_key_bytes.size() ? large_image_key_bytes.data() : nullptr;
  presence.largeImageText =
      large_image_text_bytes.size() ? large_image_text_bytes.data() : nullptr;
  presence.smallImageKey =
      small_image_key_bytes.size() ? small_image_key_bytes.data() : nullptr;
  presence.smallImageText =
      small_image_text_bytes.size() ? small_image_text_bytes.data() : nullptr;
  presence.partyId = party_id_bytes.size() ? party_id_bytes.data() : nullptr;
  presence.matchSecret =
      match_secret_bytes.size() ? match_secret_bytes.data() : nullptr;
  presence.spectateSecret =
      spectate_secret_bytes.size() ? spectate_secret_bytes.data() : nullptr;
  presence.joinSecret =
      join_secret_bytes.size() ? join_secret_bytes.data() : nullptr;
...
QDiscordRichPresence::QDiscordRichPresence()
    : StartTimestamp{NULL},
      EndTimestamp{NULL}, PartySize{NULL}, PartyMax{NULL}, Instance{NULL} {}

const QMap<QString, QtCreatorDRPCPlugin::RichPresenceFileDescriptor>
    QtCreatorDRPCPlugin::mimeTypeToRpcFileDescriptorMap = {
        ...
        {"application/vnd.qt.qmakeprofile",
         {"qmake", "QMake Project File", "Configuring"}},
        {"application/vnd.qt.qmakeproincludefile",
         {"qmake_pri", "QMake Include File", "Editing"}},
        {"application/vnd.qt.qmakeprostashfile",
         {"qt", "QMake Stash File", "Editing"}},
        ...
        {"application/x-designer",
         {"ui", "Qt User Interface File", "Designing"}},
        {"application/vnd.qt.xml.resource",
         {"qrc", "Qt Resource File", "Editing"}},
        ...
        {"text/vnd.qtcreator.git.submit",
         {"git", "Git Commit File", "Editing"}},
        {"image/png", {"image", "PNG Image", "Viewing"}}};

QMap<QString, QList<QPair<QList<QString>, QString>>>
    QtCreatorDRPCPlugin::MimeOverrider::MimeOverrideMap{
...
QString QtCreatorDRPCPlugin::MimeOverrider::OverrideMimeIfApplicable(
    const QString &mime, const Utils::FilePath &file_path) {
...
void QtCreatorDRPCPlugin::initializeDiscordRichPresence(
...
void QtCreatorDRPCPlugin::initializeControlMenu() {
  Core::ActionContainer *drpc_control_menu{
      Core::ActionManager::createMenu(Utils::Id(GLOBAL_DRPC_CONTROL_MENU_ID))};
  ...
  Core::Command *drpc_control_start_command{Core::ActionManager::registerAction(
      drpc_control_start_action,
      Utils::Id(GLOBAL_DRPC_CONTROL_MENU_START_ACTION_ID),
      Core::Context{Core::Constants::C_GLOBAL})};

  Core::Command *drpc_control_stop_command{Core::ActionManager::registerAction(
      drpc_control_stop_action, GLOBAL_DRPC_CONTROL_MENU_STOP_ACTION_ID,
      Core::Context{Core::Constants::C_GLOBAL})};

  connect(drpc_control_start_action, &QAction::triggered, this,
          &QtCreatorDRPCPlugin::activateDiscordRichPresence);

  connect(drpc_control_stop_action, &QAction::triggered, this,
          &QtCreatorDRPCPlugin::deactivateDiscordRichPresence);
...
QtCreatorDRPCPlugin::QtCreatorDRPCPlugin()
    : drpcActivatedTimestamp{NULL}, timeSpentOnCurrentEditor{NULL} {}
